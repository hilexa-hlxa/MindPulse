...
(function (root) {
   ...
   *
   * Accepts what MindPulse writes ({ phrases: [...] }), a bare array of
  ...
  function parse(raw) {
    ...
    if (!Array.isArray(incoming)) throw new Error("not a MindPulse export");

    return incoming
      .map(function (p) { return typeof p === "string" ? { text: p } : p; })
      .filter(function (p) { return p && typeof p.text === "string" && key(p.text); })
      .map(function (p) {
        return { text: key(p.text), note: p.note || "", muted: !!p.muted };
      });
  }
  ...
  function merge(existing, incoming, newId, now) {
    ...
    (incoming || []).forEach(function (p) {
      ...
      have.add(p.text);
      added.push({ id: newId(), text: p.text, note: p.note || "", muted: !!p.muted, createdAt: now });
    });
